...
const Homepage = () => {
  ...
  const handleSearch = ({ target: { value = "" } = {} }) => {
    ...
    let newData;
    if (searchedText === "") {
      newData = CelebritiesData; // Reset to original data when search text is empty
    } else {
      newData = CelebritiesData.filter(
        ({ first, last }) =>
  ...
  const handleDelete = () => {
    ...
    const updatedData = celebritiesData.filter(
      (_, index) => index !== currentCelebIndex,
    );
